...
def resumeFrag(url,start,end,num):
    try:
        import urllib.request as ur
    except:
        import urllib2 as ur
...
def downloadAll(url):
    ...
    childList=[]
    for i in range(frags):
        t=threading.Thread(target=resumeFrag,kwargs={'url':url,'start':fraglist[i][0],'end':fraglist[i][1],'num':i}) 
        t.start()
        childList.append(t)
    time.sleep(1)
